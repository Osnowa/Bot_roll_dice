...
class User(Base):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True) # ключ пользователя
    telegram_id: Mapped[int] = mapped_column(BigInteger, unique=True, nullable=False) # id пользователя в Telegram
    username: Mapped[str] = mapped_column(nullable=True) # имя пользователя в Telegram
